...
export const handleDragMove = (e: KonvaEventObject<DragEvent>, areaPoints: number[][]) => {
	...
	if (!isPointOnPath([position.x, position.y], constraints)) {
		const nearestPoint = findNearestPoint([position.x, position.y], constraints)
		draggedCircle.position(nearestPoint)
	}
}

const isPointOnPath = (point: number[], polygon: number[][]) => {
	const x = point[0]
	const y = point[1]
	...
	for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
		...
		const onPath =
			(yi === y && xi === x) || // Check if the point is one of the polygon's vertices
			(yj === y && xj === x)

		if (onPath) {
			return true // The point is on the path of the polygon
		}
	}

	return false // The point is not on the path of the polygon
}
...
const getClosestPointOnSegment = (point: number[], start: number[], end: number[]) => {
	const [px, py] = point
	const [sx, sy] = start
	const [ex, ey] = end
	...
	const t = Math.max(0, Math.min(1, ((px - sx) * (ex - sx) + (py - sy) * (ey - sy)) / lineLengthSquared))
