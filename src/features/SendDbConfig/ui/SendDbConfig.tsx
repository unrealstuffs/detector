...
import { deleteDbReplication } from '../model/services/deleteDbReplication'
...
export const SendDbConfig = (props: SendDbConfigProps) => {
	...
	const deleteDbReplicationHandler = async () => {
		const resultAction = await dispatch(deleteDbReplication())

		if (deleteDbReplication.fulfilled.match(resultAction)) {
			toast.success('Репликация удалена!')
		} else {
			toast.error(`Ошибка удаления репликации: ${resultAction.payload}`)
		}
	}
